//
//  CATAddressSelectedViewController.m
//  ZFragment
//
...
#import "CATAddressSelectedViewController.h"

@import Masonry;
@import SToolsKit;
@import JXTAlertManager;

@interface CATAddressSelectedTableViewCell : CATBaseTableViewCell

@property (nonatomic ,strong) CATAddressBean *address;
...
@end
@implementation CATAddressSelectedTableViewCell
...
- (void)setAddress:(CATAddressBean *)address {
...
@interface CATAddressSelectedViewController ()

@property (nonatomic ,strong) CATAddressBridge *bridge;

@property (nonatomic ,strong) UIButton *completeItem;

@property (nonatomic ,copy) CATAddressSelectedBlock addressBlock;

@end

@implementation CATAddressSelectedViewController

+ (instancetype)createAddressSelectedWithBlock:(CATAddressSelectedBlock) addressBlock {
    
    return [[CATAddressSelectedViewController alloc] initWithBlock:addressBlock];
}
- (instancetype)initWithBlock:(CATAddressSelectedBlock) addressBlock {
...
- (void)configOwnSubViews {
    ...
    [self.tableView registerClass:[CATAddressSelectedTableViewCell class] forCellReuseIdentifier:@"cell"];
...
- (UITableViewCell *)configTableViewCell:(id)data forIndexPath:(NSIndexPath *)ip {
    ...
    CATAddressSelectedTableViewCell *cell = [self.tableView dequeueReusableCellWithIdentifier:@"cell"];
    
    if (!cell) {
        
        cell = [[CATAddressSelectedTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"cell"];
    }
...
- (void)tableViewSelectData:(id)data forIndexPath:(NSIndexPath *)ip  {
    ...
    self.addressBlock(CATAddressActionTypeEdit, data, ip,self);
}
...
- (void)configViewModel {
    ...
    self.bridge = [CATAddressBridge new];
    // -1 失败  0 成功  1空
    ...
    } addressAction:^(enum CATAddressActionType actionType, NSIndexPath * _Nullable ip, CATAddressBean * _Nullable addressBean) {
        
        switch (actionType) {
            case CATAddressActionTypeDelete:
            {
...
- (void)alertShow:(CATAddressBean *)address andIp:(NSIndexPath *)ip {
...
- (void)insertAddress:(CATAddressBean *)addressBean {
    ...
    [self.bridge insertAddress:addressBean addressAction:^(enum CATAddressActionType actionType, NSIndexPath * _Nullable ip, CATAddressBean * _Nullable address) {
