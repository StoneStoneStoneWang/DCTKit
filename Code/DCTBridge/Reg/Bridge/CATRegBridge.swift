...
@objc(CATRegActionType)
public enum CATRegActionType: Int ,Codable {
...
public typealias CATRegAction = (_ action: CATRegActionType ) -> ()

@objc (CATRegBridge)
public final class CATRegBridge: CATBaseBridge {
    
    public var viewModel: CATRegViewModel!
}

// MARK: 201 手机号 202 密码 203 登陆按钮 204 快捷登录按钮 205 忘记密码按钮 206
extension CATRegBridge {
    
    @objc public func createReg(_ vc: CATBaseViewController ,regAction: @escaping CATRegAction) {
        ...
        if let phone = vc.view.viewWithTag(201) as? UITextField ,let vcode = vc.view.viewWithTag(202) as? UITextField ,let vcodeItem = vcode.rightView as? UIButton ,let loginItem = vc.view.viewWithTag(203) as? UIButton
            ...
            let input = CATRegViewModel.WLInput(username: phone.rx.text.orEmpty.asDriver(),
            ...
            viewModel = CATRegViewModel(input, disposed: disposed)
            // 返回序列
            backLoginItem
                .rx
                .tap
